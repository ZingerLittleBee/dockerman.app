...
Two rules ship preconfigured so you have useful coverage one click away:

| Preset | Kind | Behavior |
| --- | --- | --- |
| **Restart loop** | `restart_burst` | Fires when any container restarts ≥ 3 times within 5 minutes. |
| **Container crash (non-zero exit)** | `container_exit_non_zero` | Fires whenever any container exits with a non-zero status. |

Both presets ship **disabled** — flip them on from the Alerts page after you confirm the target scope and notification channels match your environment. Once enabled they behave like any other rule: edit fields, change the target, bind channels, or disable them again.

Presets are guarded against accidental deletion. If you remove one, a **Reset to defaults** action on the Alerts page brings it back exactly as shipped.

<Callout type="info">
  Both presets target *all containers* by default. Narrow the target to a Compose project or specific containers before enabling if your environment is noisy.
</Callout>
...
The Alerts page includes a recent-alerts feed showing every time a rule fired, with:

- Container name
- Local time of the firing
- Rule name and kind
- Context (metric value, exit code, etc.)
- Which channels received the notification

The list uses the standardized data table pattern, so column widths persist across sessions and you can search across all fields from the top toolbar.
