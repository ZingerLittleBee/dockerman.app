...
Available actions depend on the container state and runtime. Start, stop, restart, pause, unpause, rename, remove, inspect, logs, terminal, stats, clone, commit, backup, and restore are exposed where supported.
...
Enable the page's batch mode to select multiple rows. Only the actions shown in the current toolbar are supported; ordinary Ctrl/Cmd+Click and single-letter shortcuts are not a documented contract.
