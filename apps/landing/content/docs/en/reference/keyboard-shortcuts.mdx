...
description: App-defined keyboard shortcuts currently available in Dockerman.
---

Dockerman registers only the shortcuts listed below. The Command Palette shortcut is the only shortcut you can change in [Settings → Shortcut settings](/en/docs/reference/settings#shortcut-settings).
...
Focused rows and cards that expose keyboard navigation can be opened with `Enter` or `Space`. This behavior is scoped to those interactive items rather than every list in the app.
