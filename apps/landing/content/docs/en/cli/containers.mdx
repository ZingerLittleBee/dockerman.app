...
Commands for container lifecycle, streaming attachments, and backup/restore. All accept `--host` to target a specific host; otherwise the sticky host from `dockerman host use` is used.
