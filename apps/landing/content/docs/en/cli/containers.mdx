...
The `container` command group covers lifecycle operations, streaming attachments, and backup/restore. All commands accept `--host` to target a specific host; otherwise the sticky host from `dockerman host use` is used.
