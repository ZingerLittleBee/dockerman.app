...
description: Schema, call allowlist, Cloudflare Quick Tunnels, managed WSL2 engine, and scripting patterns.
---

This page covers less-common commands and patterns for wrapping `dockerman` in shell scripts, CI, or LLM tools.

## `schema` and `call`

The schema describes the supported daemon surface. `call` can invoke only its own hard-coded allowlist of unary RPCs; it is not an unrestricted escape hatch.
...
`call` expects a JSON params object. Destructive RPCs require `--yes` (or `-y`), same as the dedicated subcommands. Streaming RPCs cannot be invoked through `call`; use the dedicated streaming commands for the proper envelope.

<Callout type="info">
  `schema --format mcp-tools` emits an MCP-compatible tool registry. Feed it to an LLM agent that supports Model Context Protocol.
</Callout>
...
`tunnel create` changes network exposure, so it requires `--yes`. Pass `--install` if `cloudflared` is not yet installed; the CLI streams installation progress before opening the tunnel. SSH-forwarded remote Docker hosts are not supported as tunnel targets.
...
`host current` is local-only and cannot test daemon reachability. Use a daemon-backed command such as `system info`; discovery and handshake failures exit with code `3`.
