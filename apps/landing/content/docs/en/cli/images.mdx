...
`image`, `hub`, and `trivy` cover Hub discovery, pull/push, build, CVE scanning, and cleanup.
...
`image search` queries the Docker Engine's `/images/search` endpoint via the connected daemon. For descriptions, official badges, and tag listings, use [`hub search`](#search-docker-hub) below. The two commands talk to different APIs.
