...
`dockerman` is a thin command-line client that talks to the local **dockerman-daemon** over an authenticated IPC socket. It covers a substantial, explicitly allowlisted subset of Docker, Compose, read-only Kubernetes, Trivy, Docker Hub, tunnels, and WSL workflows. It does not mirror every GUI feature. Added in v5.3.0.
