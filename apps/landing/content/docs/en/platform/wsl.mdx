---
title: Windows WSL
description: Connect Dockerman to Docker Desktop's WSL 2 backend or to Docker Engine running inside WSL 2.
---

Dockerman works with Docker under Windows Subsystem for Linux (WSL 2).
...
WSL 2 runs inside a virtual machine. Keep project files on the WSL 2 filesystem (not on a Windows drive), and allocate enough memory to WSL 2 in `.wslconfig`.
