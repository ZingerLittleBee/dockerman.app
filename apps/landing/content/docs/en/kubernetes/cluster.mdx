...
Start a local k3d cluster or import an existing kubeconfig, then work from the Kubernetes section.
...
<Steps>
  ...
  <Step>
    ...
    Dockerman parses the file and lists its contexts. Pick the context you want to import; the current flow does not promise separate pre-filled server URL and user-binding fields.
  </Step>
  <Step>
...
The Cluster page shows the API server version, connection state, and node information. Kubernetes Node objects expose kubelet and container-runtime versions, not a per-node kube-proxy version.
...
- **Nodes** — node list with conditions, capacity, allocatable resources, labels, kubelet version, container runtime, and scheduled Pods.
...
Open the dedicated [Events](/en/docs/kubernetes/events) browser to filter by namespace, type (Normal/Warning), reason, and involved object before you narrow to a single resource.
