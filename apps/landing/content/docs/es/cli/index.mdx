...
`dockerman` es un cliente de línea de comandos ligero que se comunica con el **dockerman-daemon** local a través de un socket IPC autenticado. La CLI sólo expone una lista explícita de operaciones y no equivale a toda la GUI.
...
| Flag | Propósito |
| --- | --- |
| `--host <id>` | Apunta a un host Docker concreto. Usa `LOCALHOST` para el socket local. `dockerman host current` sólo muestra la selección actual. |
