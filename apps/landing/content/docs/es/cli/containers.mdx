...
`--config` acepta un objeto JSON inline, no una ruta de fichero. `commit` crea una imagen desde el contenedor.
...
<Steps>
  ...
  <Step>
    ...
    Usa `--include-bind-mount <índice>` para elegir mounts y `--no-pause` para saltarte pause/unpause. `--force` sólo sobrescribe un archivo de salida existente.
  </Step>
  <Step>
