...
El grupo `k8s` es **sólo lectura**: descubre clusters e inspecciona una lista explícita de recursos integrados. No crea, actualiza ni elimina recursos, ni consulta tipos CR arbitrarios.
...
El subcomando `resource` sólo admite una lista fija de identificadores. `api-resource list` muestra discovery del cluster, pero no vuelve invocable cada tipo descubierto.
