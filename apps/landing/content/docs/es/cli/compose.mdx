...
`compose up` acepta `--detach` (por defecto) o `--attach`. Actualmente la salida attach se devuelve al terminar el proceso, no como logs en tiempo real.
...
`deploy` toma un nombre de proyecto y un fichero compose. El daemon lo ejecuta desde un directorio temporal, por lo que contextos de build, bind mounts o env files relativos pueden fallar. Usa rutas absolutas o `up`.
