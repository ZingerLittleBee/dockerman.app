...
Los grupos `image`, `hub` y `trivy` cubren juntos el ciclo de vida de las imágenes: descubrir en Docker Hub, hacer pull, escanear CVEs, build, push y limpieza.
...
Dockerman puede gestionar el binario de Trivy. Un escaneo aún puede necesitar descargar o actualizar la base de datos, así que no se garantiza el uso offline.
...
<Steps>
  ...
  <Step>
    ...
    Instalan o actualizan el binario gestionado de Trivy; no garantizan que la base de datos se descargue por adelantado.
  </Step>
  <Step>
    ### Escanear una imagen
