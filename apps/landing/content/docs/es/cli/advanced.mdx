...
Esta página reúne los comandos CLI menos frecuentes y los patrones que usamos al envolver `dockerman` desde scripts de shell, pipelines de CI o herramientas LLM.

## `schema` y `call`

El schema describe la superficie soportada del daemon. `call` solo invoca su lista fija de RPC unary; no es un escape hatch sin restricciones.
...
dockerman call hub_search '{"query":"alpine","pageSize":5}'
dockerman call remove_container '{"name":"web","options":{"force":true}}' --yes
```

`call` espera un objeto JSON de params. Los RPCs destructivos requieren `--yes` (o `-y`) igual que los subcomandos dedicados. **Los RPCs en streaming no se pueden invocar con `call`**: usa los comandos de streaming dedicados para obtener el envelope correcto.

<Callout type="info">
  `schema --format mcp-tools` emite un registro de herramientas compatible con MCP. Conéctalo a un agente LLM que ya hable Model Context Protocol.
</Callout>
...
`events` es un comando en streaming y siempre emite [envelopes](/es/docs/cli/streaming) NDJSON en stdout; no hay flag `--json` porque no hay modo plano. Repite `--filter key=value` para filtros adicionales; la coincidencia usa la semántica estándar de filtros de Docker. `--since` y `--until` aceptan timestamps Unix o datetimes RFC 3339.
...
`tunnel create` es destructivo en el sentido de que cambia la exposición de red, así que requiere `--yes`. Pasa `--install` si `cloudflared` aún no está instalado; la CLI transmite el progreso de instalación antes de abrir el túnel. Los hosts Docker remotos reenviados por SSH no están soportados como destino de túneles.

`--host-port` y `--all` son mutuamente excluyentes: o eliges puertos publicados concretos o expones cada puerto que el contenedor publica. `--host-ip` requiere al menos un `--host-port`; no puedes hacer bind por IP sin nombrar el puerto.
...
`wsl unregister --yes` borra la distro gestionada **y todos los contenedores, imágenes y volúmenes que contiene**. No hay marcha atrás; úsalo solo cuando estés seguro.
...
```bash
dockerman container list | jq '.[] | select(.state=="running") | .name'
...
La CLI siempre exige `--yes`, por muy confiado que esté el script. **No existe** `--force-yes` ni un bypass por variable de entorno; es intencional.

### Detectar fallo de descubrimiento del daemon

```bash
if ! dockerman system info >/dev/null 2>&1; then
...
Los runners de CI suelen matar procesos que no producen salida durante unos minutos. El heartbeat de 15s del daemon hace que los comandos de streaming siempre emitan *algo* por el cable, pero el modo plano oculta los heartbeats. Usa `--json` en CI para que los heartbeats salgan como líneas visibles, o canaliza por `ts` para añadir marcas de tiempo a cada línea.
