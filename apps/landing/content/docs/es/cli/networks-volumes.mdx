...
Los grupos `network` y `volume` exponen las operaciones que la GUI ofrece para estos dos tipos de recursos. Ambos siguen los mismos patrones que `container` e `image`: `list`, `inspect`, `create`, `remove`/`prune`, más comandos de relación para redes.
...
| `--attachable` | Permite que contenedores standalone se conecten (solo overlay) |
...
`--force` se reenvía a la API de Docker. Que sea necesario depende del daemon y del estado de la red.
...
`remove` requiere `--yes`. `--force` se reenvía a Docker, pero no garantiza eliminar un volumen todavía referenciado.
