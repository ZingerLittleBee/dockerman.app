...
| `system.json` | Versión de Docker e información del sistema. Se elimina el usuario del registro, pero no se aplica una limpieza general de secretos. |
| `events.json` | Eventos de Docker filtrados por la ventana de tiempo seleccionada y contenedores objetivo |
| `inspect.json` | Datos de inspect de contenedor, imagen, red y volumen. Por defecto solo se redactan los valores con nombre coincidente en la lista `Env` de la configuración del contenedor. Incluye `State.Health`. |
