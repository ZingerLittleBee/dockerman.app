...
`container` 命令组覆盖容器生命周期、流式附加，以及备份/恢复。所有命令都接受 `--host` 指定目标主机；未指定时使用 `dockerman host use` 设置的默认主机。
...
`--config` 接受内联 JSON 对象，不接受文件路径。`commit` 把运行中的容器快照成新镜像。
...
<Steps>
  <Step>
    ...
    列出归档可包含的卷和 bind mount。纳入 bind mount 前应自行核对其内容和大小。
  </Step>
  <Step>
    ### 创建归档
    ...
    使用 `--include-bind-mount <索引>` 单独选择 mount，`--no-pause` 跳过暂停/恢复。`--force` 只覆盖已存在的输出归档。
  </Step>
  <Step>
    ### 预览归档
...
<Callout type="warn">
  备份路径在 CLI 端解析为绝对本地路径后才发给 daemon。daemon 要求绝对路径，但不会替你判断 bind mount 是否属于“安全工作区”。
</Callout>
