...
`dockerman` 是一个轻量级命令行客户端，它通过带身份验证的 IPC 套接字与本地 **dockerman-daemon** 通信。CLI 只暴露经过明确允许的一组操作，并不等同于完整 GUI。
...
如果你通过 AI 助手驱动 `dockerman`，可安装官方 plugin 提供命令地图和安全提示。plugin 可能落后于已安装应用，自动化前仍应以当前版本的 `dockerman --help`、`dockerman schema` 和实际输出为准。
