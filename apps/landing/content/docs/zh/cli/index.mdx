...
`dockerman` 是命令行客户端，通过带身份验证的 IPC 套接字与本地 **dockerman-daemon** 通信。CLI 只暴露经过明确允许的一组操作，并不等同于完整 GUI。

<Callout type="info">
  CLI 会按需自动启动 `dockerman-daemon`，不必打开 GUI。
</Callout>
...
<Steps>
  <Step>
    ...
    按 [快速开始](/zh/docs/getting-started) 安装对应平台的 GUI（macOS DMG、Windows MSI 或 Linux 包）。
  </Step>
  <Step>
    ### 为当前用户启用 CLI
...
| 参数 | 用途 |
| --- | --- |
| `--host <id>` | 指定 Docker 主机。本地套接字使用 `LOCALHOST`；`dockerman host current` 只显示当前选择。 |
...
如果通过 AI 助手调用 `dockerman`，可安装官方 plugin 作为命令地图和安全提示。plugin 可能落后于已安装应用，自动化前仍以当前版本的 `dockerman --help`、`dockerman schema` 和实际输出为准。
...
然后在 Codex 中运行 `/plugins`，从 **Dockerman** marketplace 安装并启用 **Dockerman** plugin。详情和锁定版本的方法见 [AI 助手 Plugin](/zh/docs/cli/skills)。
