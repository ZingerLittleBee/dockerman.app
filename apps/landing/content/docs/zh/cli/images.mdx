...
`image`、`hub`、`trivy` 三个命令组覆盖镜像生命周期：在 Docker Hub 上发现镜像、拉取、扫描 CVE、构建、推送、清理。
...
Dockerman 可管理 Trivy 二进制。扫描仍可能需要下载或更新漏洞数据库，不承诺安装后二进制即可离线扫描。
...
<Steps>
  ...
  <Step>
    ...
    两者安装或更新受管的 Trivy 二进制；不会预先保证漏洞数据库已下载。
  </Step>
  <Step>
    ### 扫描镜像
