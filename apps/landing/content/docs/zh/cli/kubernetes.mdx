...
`k8s` 命令组是**只读**的：从 kubeconfig 发现集群、切换，并查看明确允许的一组内置资源。它不能查询任意自定义资源类型。
...
返回 node、pod、deployment、service 的计数，以及最近事件。事件列表并不只包含 Warning。
...
通用 `resource` 子命令只支持固定的资源标识允许列表。`api-resource list` 可以显示集群发现数据，但发现到的类型不会因此自动变得可调用：
...
<Callout type="warn">
  所有 k8s 命令按设计只读。需要修改资源时请直接使用 `kubectl`。Dockerman 的 k8s 接口给自动化和 GUI 里的安全只读场景用。
</Callout>
