...
3. 对于每个订阅的镜像，Dockerman 通过**凭证解析器**解析出一个仓库凭证，然后比较本地 digest 和远程镜像仓库 digest。模糊的凭证会**强制失败** — 检查会报错，而不是猜测。
4. 当发现新 digest 时，Dockerman 通过你配置的[通知渠道](/zh/docs/homelab/notifications)发送一条聚合通知，并写入历史游标，让下一次运行从本次结束的位置继续。
