...
`compose remove`（`compose down` の別名）は破壊的なので `--yes` が必須です。`--remove-volumes` は Compose が認識したボリュームの削除を要求しますが、名前付きボリュームをすべて列挙する保証はありません。
...
`deploy` はプロジェクト名と compose ファイルを取ります。デーモンは一時ディレクトリで実行するため、相対的な build context、bind mount、env file は失敗することがあります。絶対パスか、既存プロジェクトの `up` を使ってください。
