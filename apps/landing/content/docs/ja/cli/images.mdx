...
`image search` は接続先のデーモン経由で Docker Engine の `/images/search` エンドポイントを叩きます。説明、公式バッジ、タグ一覧など、より詳しい情報が必要なら、下の [`hub search`](#docker-hub-を検索) を使ってください。両者は別の API と通信します。
...
Dockerman は Trivy バイナリを管理できます。スキャン時には脆弱性データベースの取得や更新が必要な場合があり、オフライン動作は保証されません。
...
<Steps>
  ...
  <Step>
    ...
    管理対象の Trivy バイナリをインストールまたは更新します。脆弱性データベースの事前ダウンロードは保証しません。
  </Step>
  <Step>
    ### イメージをスキャン
