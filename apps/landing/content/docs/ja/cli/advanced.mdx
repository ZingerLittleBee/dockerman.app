...
使用頻度の低い CLI コマンドと、シェルスクリプト、CI パイプライン、LLM ツールチェーンから `dockerman` を包むときに使うパターンをまとめます。

## `schema` と `call`

schema は対応するデーモン面を記述します。`call` が呼べるのは、ハードコードされた allowlist の unary RPC だけです。無制限の逃げ道ではありません。
