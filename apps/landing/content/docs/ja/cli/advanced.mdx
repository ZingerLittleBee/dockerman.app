...
使用頻度の低い CLI コマンドと、シェルスクリプト、CI パイプライン、LLM ツールチェーンから `dockerman` をラップするときに使うパターンをまとめます。

## `schema` と `call`

schema は対応するデーモン側の API 面を記述します。`call` が呼べるのは、ハードコードされた許可リスト上の unary RPC だけです。無制限のエスケープハッチではありません。
...
`call` は JSON 形式のパラメータオブジェクトを受け取ります。破壊的 RPC は専用サブコマンドと同じく `--yes`（または `-y`）が必要です。**ストリーミング RPC は `call` で呼べません**——正しいエンベロープを得るには専用のストリーミングコマンドを使ってください。

<Callout type="info">
  `schema --format mcp-tools` は MCP 互換のツールレジストリを出力します。Model Context Protocol を話す LLM エージェントにそのまま渡せます。
</Callout>
...
ローカルコンテナのポートをトンネル経由で公開し、一時的な `trycloudflare.com` URL を発行します。Quick Tunnels はその場限りで認証なし——開発やライブデモ用途に限り、本番の公開入口としては使わないでください。
...
`tunnel create` はネットワークの公開範囲が変わるため、破壊的な操作として扱われ、`--yes` が必須です。`cloudflared` が未インストールなら `--install` を付けてください；CLI はインストール進捗をストリーミングしてからトンネルを開きます。SSH 転送のリモート Docker ホストはトンネルターゲットとしてサポートされません。
...
Windows では、デーモンが `dockerman-backend` という専用 WSL2 ディストリを管理し、その中で Docker Engine を動かして Linux コンテナを実行できます。CLI と GUI は同一エンジンを共有します。
...
デーモン発見とハンドシェイク失敗の終了コードは `3` で、RPC レベルの失敗（`1`）やストリーム失敗（`4`）とは別です。

### CI での長時間ストリーム

CI ランナーは数分間出力がないプロセスを強制終了しがちです。デーモンの 15 秒ハートビートは、ストリーミングコマンドが通信路上に何らかの出力を流すことを保証しますが、平文モードではハートビートは隠れます。CI では `--json` を使ってハートビートを可視行として現すか、`ts` にパイプして各行にタイムスタンプを付けてください。
