...
`k8s` コマンド群は**読み取り専用**です。kubeconfig からクラスタを発見し、明示的に許可された built-in resource を閲覧します。任意の CR kind は照会できません。
...
node、pod、deployment、service の数と最近の event を返します。event は Warning だけに限定されません。
...
汎用 `resource` サブコマンドも固定 allowlist だけをサポートします。`api-resource list` の discovery 結果が、そのまま callable になるわけではありません。
