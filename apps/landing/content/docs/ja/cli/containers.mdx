...
<Steps>
  <Step>
    ...
    アーカイブに含められる volume と bind mount を一覧します。内容とサイズは自分で確認してください。
  </Step>
  <Step>
    ### アーカイブ作成
...
<Callout type="warn">
  バックアップパスは CLI 側で絶対ローカルパスに解決されてから daemon に送られます。daemon は絶対パスを要求しますが、bind mount が「安全な workspace」内かどうかは判定しません。
</Callout>
